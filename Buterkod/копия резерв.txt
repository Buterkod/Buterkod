...
import subprocess
import sys
from PyQt6.QtCore import QPoint, QPointF, QSharedMemory, Qt, QTimer, QUrl
...
for directory in [GAMES_DIR, IMAGES_DIR, MUSIC_DIR]:
...
class MusicToggleButton(QPushButton):
  ...
  def update_state_visual(self):
    if self.main_window.audio_output.isMuted():
      self.setText("🔇")
    else:
      self.setText("🔊")
...
class DownloadableGameCard(QFrame):
  ...
  def __init__(self, title, desc, action_text, callback, is_dark=False):
    ...
    if os.path.exists(img_path):
      ...
      self.img_lbl.setText("🖼️ Нет картинки")
    ...
    self.d_lbl.setStyleSheet(
        f"font-size: 12px; color: {'#bbb' if is_dark else '#666'}; border: none;"
    )
    self.d_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
    self.d_lbl.setWordWrap(True)
    layout.addWidget(self.d_lbl)

    btn_container = QVBoxLayout()
    btn_container.setSpacing(2)
    self.action_btn = QPushButton(action_text)
    self.action_btn.setStyleSheet(
        "background: #4CAF50; color: white; padding: 6px 20px; border-radius:"
        " 6px; font-weight: bold; font-size: 13px;"
    )
  ...
  def resizeEvent(self, event):
    ...
    self.progress_bar.setGeometry(0, self.height() - 5, w, 5)
  ...
  def on_btn_clicked(self):
    if self.action_btn.text() == "Скачать" and not self.is_downloading:
      ...
      self.action_btn.setStyleSheet(
          "background: #a5d6a7; color: white; padding: 6px 20px; border-radius:"
          " 6px; font-weight: bold; font-size: 13px;"
      )
...
class MainScreen(QWidget):
  ...
  def __init__(self, main_window):
    ...
    shop_layout.addWidget(shop_title, alignment=Qt.AlignmentFlag.AlignCenter)
    shop_layout.addSpacing(25)
    ...
    self.shop_container = QFrame()
    self.shop_container_layout = QVBoxLayout(self.shop_container)
    self.shop_container_layout.setContentsMargins(25, 25, 25, 25)

    self.shop_grid_layout = QGridLayout()
    self.shop_grid_layout.setSpacing(20)
    self.shop_container_layout.addLayout(self.shop_grid_layout)

    shop_layout.addWidget(self.shop_container)

    max_columns = 3

    for index, (title, desc, filename) in enumerate(self.games_list):
      card = DownloadableGameCard(
          title,
          desc,
          "Купить: 0 руб",
          lambda t=title: self.buy_game(t),
          self.is_dark_theme(),
      )
      try:
        card.action_btn.clicked.disconnect()
      except TypeError:
        pass
      card.action_btn.clicked.connect(lambda _, t=title: self.buy_game(t))
      self.shop_buttons[title] = card.action_btn
      self.shop_card_frames.append(card)
    ...
    for btn in [self.cat_friends_btn, self.cat_add_btn, self.cat_req_btn]:
    ...
    send_req_btn.setFixedHeight(38)
    send_req_btn.setStyleSheet(
        "background: #4CAF50; color: white; font-weight: bold; padding: 0px"
  ...
  def apply_theme(self):
    ...
    if dark:
      ...
      self.shop_container.setStyleSheet(
    ...
    self.recreate_shop_cards()

  def recreate_shop_cards(self):
    while self.shop_grid_layout.count():
      item = self.shop_grid_layout.takeAt(0)
      if item.widget():
        item.widget().deleteLater()

    self.shop_buttons.clear()
    self.shop_card_frames.clear()
    max_columns = 3
  ...
  def switch_friends_subtab(self, index):
    ...
    active = (
        ...
        " 6px; padding: 0px 16px;"
    )
    inactive = (
        "background: #333; color: #ccc; font-weight: bold; border: 1px solid"
        " #555; border-radius: 6px; padding: 0px 16px;"
        if self.is_dark_theme()
        else "background: white; color: #333; font-weight: bold; border: 1px solid #ccc; border-radius: 6px; padding: 0px 16px;"
    )
  ...
  def refresh_shop_view(self):
    if not self.main_window.current_user:
      return
    self.main_window.users_db = load_users()
    ...
    for title, btn in self.shop_buttons.items():
      if title in library_games:
        ...
        btn.setStyleSheet(
            "background: #ccc; color: #666; padding: 6px 20px; border-radius:"
            " 6px; font-weight: bold; font-size: 13px;"
        )
      else:
  ...
  def refresh_friends_view(self):
    ...
    if not requests:
      ...
      for req_nick in requests:
        ...
        info_lbl.setStyleSheet(
            f"font-size: 14px; border: none; color: {'white' if dark else 'black'};"
        )
        ...
        accept_btn.setStyleSheet(
            ...
            " 0px 10px; font-weight: bold; font-size: 12px;"
        )
        ...
        decline_btn.setFixedHeight(30)
        decline_btn.setStyleSheet(
            "background: #888; color: white; border-radius: 4px; padding: 0px"
            " 10px; font-size: 12px;"
        )
  ...
  def toggle_profile_menu(self):
    ...
    global_pos = self.profile_btn.mapToGlobal(
        QPoint(0, self.profile_btn.height())
    )
...
class SettingsScreen(QWidget):
  ...
  def __init__(self, main_window):
    ...
    self.avatar_input.setStyleSheet("font-size: 18px;")
...
class MainWindow(QStackedWidget):
  ...
  def __init__(self):
    ...
    music_path = os.path.join(MUSIC_DIR, "Buterkod_music.mp3")
...
if __name__ == "__main__":
  ...
  shared_memory = QSharedMemory("Buterkod_App_Unique_Key_12345")
